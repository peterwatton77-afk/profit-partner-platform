...
import { ArrowRight, Crosshair, Calculator, BarChart3, Star, Shield, Zap, Check } from "lucide-react";
...
const brandConfigs = {
  oddsmonkey: {
    ...
    cssClass: "",
    stats: [{ v: "45K+", l: "Members" }, { v: "£1.5K+", l: "Avg Profit" }, { v: "£28M+", l: "Total" }],
  },
  outplayed: {
    name: "Outplayed",
    logo: () => <span className="font-display text-lg font-bold text-gradient">Outplayed</span>,
    headline: "Beat the Bookies, Keep the Profits",
    badge: "Smart Betting for Sports Fans",
    cssClass: "brand-outplayed",
    stats: [{ v: "28K+", l: "Members" }, { v: "£1.2K+", l: "Avg Profit" }, { v: "£18M+", l: "Total" }],
  },
};
      ...
      <div className="glass-card overflow-hidden">
        ...
        <div className="p-6 sm:p-8 relative">
          ...
          <div className="relative text-center">
            ...
            <h3 className="font-display text-lg sm:text-xl font-bold leading-tight mb-3">
              ...
              <span className="text-gradient">{b.headline.split(" ").pop()}</span>
            </h3>
        ...
        <div className="px-6 pb-4">
          <div className="grid grid-cols-2 gap-2">
            {[{ icon: Crosshair, name: "OddsMatcher" }, { icon: Calculator, name: "Calculators" }, { icon: BarChart3, name: "Profit Tracker" }, { icon: Shield, name: "Risk-Free" }].map((t, i) => (
              <div key={i} className="flex items-center gap-2 p-2.5 rounded-lg border border-border/50 bg-background/30">
                <div className="w-7 h-7 rounded bg-primary/10 flex items-center justify-center shrink-0"><t.icon size={13} className="text-primary" /></div>
        ...
        <div className="px-6 pb-6">
          <div className="flex gap-2">
            <div className="flex-1 glass-card p-3 text-center">
              ...
              <div className="mt-2 space-y-1">{["Guides", "Limited tools"].map((f) => <div key={f} className="flex items-center gap-1 text-[9px] text-muted-foreground"><Check size={8} className="text-primary" />{f}</div>)}</div>
            </div>
            <div className="flex-1 glass-card glow-border p-3 text-center">
              <p className="text-xs font-semibold">Premium</p>
              <p className="font-display text-lg font-bold">£17.99</p>
              <div className="mt-2 space-y-1">{["All tools", "Full access"].map((f) => <div key={f} className="flex items-center gap-1 text-[9px] text-muted-foreground"><Check size={8} className="text-primary" />{f}</div>)}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <header className="border-b border-border bg-card/50 sticky top-0 z-50 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Same codebase, different brands — adding a new brand is just a config entry</p>
          </div>
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-3 bg-secondary/50 border border-border rounded-lg px-4 py-2">
              <Label className={`text-sm font-medium transition-colors ${!showOutplayed ? "text-[hsl(152_72%_46%)]" : "text-muted-foreground"}`}>OddsMonkey</Label>
              <Switch checked={showOutplayed} onCheckedChange={setShowOutplayed} />
              <Label className={`text-sm font-medium transition-colors ${showOutplayed ? "text-[hsl(245_58%_63%)]" : "text-muted-foreground"}`}>Outplayed</Label>
            </div>
            <Button variant="outline" size="sm" className="text-xs border-border" asChild><Link to="/">← Back Home</Link></Button>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-8">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            <div className="flex-1"><p className="text-xs font-semibold text-[hsl(152_72%_46%)] mb-2">OddsMonkey (Green)</p><PreviewCard brandId="oddsmonkey" /></div>
            <div className="flex-1"><p className="text-xs font-semibold text-[hsl(245_58%_63%)] mb-2">Outplayed (Purple)</p><PreviewCard brandId="outplayed" /></div>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass-card p-6">
          <h3 className="font-display text-sm font-semibold mb-3">White-Label Architecture</h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs text-muted-foreground">
            <div><p className="font-medium text-foreground mb-1">CSS Variable Theming</p><p>Brand themes override <code className="text-primary">--primary</code>, <code className="text-primary">--gradient-hero</code>, and glow tokens via a single CSS class. All components automatically inherit.</p></div>
            <div><p className="font-medium text-foreground mb-1">BrandProvider Context</p><p>React context provides brand config (name, copy, logo, stats) to all components. URL-based detection auto-switches brands on <code className="text-primary">/outplayed/*</code> routes.</p></div>
            <div><p className="font-medium text-foreground mb-1">Adding a New Brand</p><p>1) Add CSS class with color overrides. 2) Add config object to brands map. 3) Add URL prefix detection. Same codebase, infinite brand deployments.</p></div>
          </div>
        </motion.div>
      </div>
    </div>
